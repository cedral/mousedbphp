...
class LitterController extends mdb_Controller {
	...
	public function weanAction() {
		...
		$wean_action = $formData['wean_action'];
		
		if ($wean_action == 'not_viable') {
...
//					$cageRow->set_up_on = date('Y-m-d');
			...
			throw new Exception('/litter/wean called with unrecognised option '.$wean_action);
		}
		$this->_redirect ( '/litter/view/id/' . $id );
